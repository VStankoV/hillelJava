...
/*		System.out.print("Enter first SORTED ");
	...
	*/
		...
		int[] arrayResult = new int[array1.length + array2.length+10];
		...
		for (int i : array1) {
			...
			try {
				while (array1[i] > array2[pos2]) {			//array2[pos2] вызовет outOfBoundExeption когда закончится
					arrayResult[posR] = array2[pos2++];
					posR++;
				}
			} catch (ArrayIndexOutOfBoundsException e){
				arrayResult[posR] = array1[i];
				posR++;
				continue;
			}
			arrayResult[posR++] = array1[i];
